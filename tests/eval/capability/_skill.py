...
SERVER_MODULE = "vmware_avi.mcp_server.server"
...
#: ``pool``, ``server`` and ``namespace`` are bare nouns rather than suffixed
#: identifiers, so they are matched whole.
ENTITY_WORDS = {
    'vs': ('vs', 'virtual_service', 'virtualservice'),
    'pool': ('pool', 'pools'),
    'server': ('server', 'servers', 'member', 'members'),
    'namespace': ('namespace', 'namespaces'),
    # ako_ingress_diagnose takes an ingress name. Nothing on this surface
    # enumerates ingresses, which is a finding rather than a reason to leave
    # the parameter unclassified.
