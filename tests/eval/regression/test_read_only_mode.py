...
def _load_server(monkeypatch, read_only: str | None):
    """Import vmware_avi.mcp_server.server fresh under the given read-only env."""
    ...
    for name in [m for m in sys.modules if m.startswith("vmware_avi.mcp_server")]:
        del sys.modules[name]
    return importlib.import_module("vmware_avi.mcp_server.server")
...
def test_skill_env_var_also_works(monkeypatch):
    ...
    for name in [m for m in sys.modules if m.startswith("vmware_avi.mcp_server")]:
        del sys.modules[name]
    server = importlib.import_module("vmware_avi.mcp_server.server")
